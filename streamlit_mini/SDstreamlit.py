...
DEFAULT_PROMPT = "starcraft2 protoss"
DEFAULT_WIDTH, DEFAULT_HEIGHT = 320, 320
...
def prompt_and_generate_button(prefix, pipeline_name: PIPELINE_NAMES, **kwargs):
    ...
    step_epoch = st.slider("Number of inference steps", min_value=1, max_value=200, value=25, key=f'slider_1_{prefix}')
    guidance_scale = st.slider(
        "Guidance scale", min_value=0.0, max_value=20.0, value=7.5, step=0.5, key=f'GSslider_1_{prefix}'
    )
...
def new1_prompt_and_generate_button(prefix, pipeline_name: PIPELINE_NAMES, **kwargs):
    prompt = st.text_area(
        "Prompt",
        value=DEFAULT_PROMPT,
        key=f"{prefix}-prompt",
    ...
    step_epoch1 = st.slider("Number of inference steps", min_value=1, max_value=200, value=25, key='slidernew1')
    guidance_scale = st.slider(
        "Guidance scale", min_value=0.0, max_value=20.0, value=7.5, step=0.5, key='GSslidernew1'
    )
...
def new2_prompt_and_generate_button(prefix, pipeline_name: PIPELINE_NAMES, **kwargs):
    prompt = st.text_area(
        "Prompt",
        value=DEFAULT_PROMPT,
        key=f"{prefix}-prompt",
    ...
    step_epoch2 = st.slider("Number of inference steps", min_value=1, max_value=200, value=25, key='slidernew2')
    guidance_scale = st.slider(
        "Guidance scale", min_value=0.0, max_value=20.0, value=7.5, step=0.5, key='GSslidernew2'
    )
...
def inpainting_tab():
    ...
    with col2:
        if image_input and mask_input:
            new1_prompt_and_generate_button(
...
def img2img_tab():
    ...
    with col2:
        if image:
            new2_prompt_and_generate_button("img2img", "img2img", image_input=image)
